...
const EditFormCategory = ()=>{
  ...
  useEffect(()=>{
    getCategoryByID(id).then((response)=> setCategory(response.data)).catch((error)=>console.error(error));
  },[]);
  ...
  const handleSubmit = async (event) => {
    ...
    const h = await editCategory(category,id).then((response) => {
    ...
    if(h.status === 200) {
      setResponse(true);
      setMessageTrue('Categoria modificada correactamente')
    }else{
      setErrors(true)
